...
const VideoDetails = () => {
  ...
  useEffect(() => {
    const fetchVideoData = async () => {
      try {
        const response = await fetch(`http://localhost:4000/api/videos/${id}`); // Adjust the URL to your backend API
        if (!response.ok) {
  ...
  const handleCommentSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:4000/api/videos/${id}/comments`, {
        method: 'POST',
        headers: {
